...
class _TabSelesaiState extends State<TabSelesai> {
  @override
  void initState() {
    context.read<GetMissionsSelesaiCubit>().getMissionsSelesai();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        ...
        await context.read<GetMissionsSelesaiCubit>().getMissionsSelesai();
      },
      child: Column(
        ...
        children: [
          ...
          Expanded(
            child:
                BlocBuilder<GetMissionsSelesaiCubit, GetMissionsSelesaiState>(
              builder: (context, state) {
                if (state is GetMissionsSelesaiLoaded) {
                  return state.missionsSelesai.data?.length == 0
                      ...
                      : ListView.builder(
                          ...
                          itemCount: state.missionsSelesai.data?.length,
                          itemBuilder: (context, index) {
                            var data = state.missionsSelesai.data?[index];
                            return Column(
                              children: [
                                MissionCard(
                ...
                } else if (state is GetMissionsSelesaiLoading) {
                  return Center(
